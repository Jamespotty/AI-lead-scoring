    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "Classify each lead. Return a JSON array.\n\n" + json.dumps(payload, ensure_ascii=False)},
            ],
            temperature=0,
            max_tokens=1500,
        )
        ...
        fallback = [{"id": lead["_lead_id"], "intent": "low", "score": 0, "reason": "Scoring error"} for lead in batch]
        return fallback, [err]
...
def run_scoring_pipeline(
    ...
    all_batches = [leads[i: i + SCORE_BATCH_SIZE] for i in range(0, len(leads), SCORE_BATCH_SIZE)]
    pending = [b for b in all_batches if not all(lead["_lead_id"] in score_map for lead in b)]
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(score_batch, batch, openai_key, col_title, col_body, col_author, col_engagement, system_prompt): batch
    ...
    scored.sort(key=lambda x: (intent_order.get(x.get("intent", "low"), 2), -x.get("score", 0)))
...
def render_sidebar():
    with st.sidebar:
        ...
        for page in ["Dashboard", "Upload Data", "ICP Configuration", "Lead Scoring", "Results & Export", "Settings"]:
...
def page_dashboard():
    ...
    if st.session_state.activity_log:
        st.dataframe(pd.DataFrame(st.session_state.activity_log), use_container_width=True, hide_index=True)
    else:
...
def page_upload_data():
    ...
    c1, c2 = st.columns(2)
    with c1:
        col_title     = st.selectbox("Post Title",              cols_with_blank, index=best_guess(["title", "subject", "headline"], cols_with_blank))
        col_body      = st.selectbox("Post Body / Comment *",   cols_with_blank, index=best_guess(["body", "text", "content", "selftext", "comment"], cols_with_blank))
        col_permalink = st.selectbox("Permalink / URL",         cols_with_blank, index=best_guess(["permalink", "url", "link", "post_url"], cols_with_blank))
    with c2:
        col_author     = st.selectbox("Username / Author",      cols_with_blank, index=best_guess(["author", "username", "user", "handle", "screen_name"], cols_with_blank))
        col_engagement = st.selectbox("Engagement Score",       cols_with_blank, index=best_guess(["score", "upvotes", "likes", "views", "engagement"], cols_with_blank))

    def norm(v): return "" if v == "(none)" else v
    ...
    if st.session_state.col_title or st.session_state.col_body:
        with st.expander("🔍 Preview: what the model sees (first 3 rows)"):
            for i, row in enumerate(records[:3], 1):
                title_val = str(row.get(st.session_state.col_title, ""))[:200] if st.session_state.col_title else ""
                body_val  = str(row.get(st.session_state.col_body,  ""))[:TEXT_FIELD_LIMIT] if st.session_state.col_body else ""
                combined  = f"{title_val} | {body_val}" if title_val and body_val else (title_val or body_val)
                author    = str(row.get(st.session_state.col_author, ""))[:80] if st.session_state.col_author else "—"
                link      = str(row.get(st.session_state.col_permalink, ""))[:120] if st.session_state.col_permalink else "—"
                st.markdown(f"**Row {i}**")
...
def page_icp_configuration():
    ...
    st.session_state.icp_product_description = st.text_area(
        ...
        value=st.session_state.icp_product_description, height=100,
    ...
    st.session_state.icp_target_customer = st.text_area(
        ...
        value=st.session_state.icp_target_customer, height=80,
    ...
    st.session_state.icp_pain_points = st.text_area(
        ...
        value=st.session_state.icp_pain_points, height=80,
...
def page_lead_scoring():
    ...
    st.caption(f"Batch: {SCORE_BATCH_SIZE}/call · {MAX_WORKERS} workers · checkpoint every {CHECKPOINT_EVERY} batches")
    ...
    if has_checkpoint:
        ...
        st.info(f"♻️ **Interrupted run detected** — {prior_count:,} / {n:,} scored ({prior_count/n*100:.0f}%). {n-prior_count:,} remain.")
        col_r, col_d = st.columns(2)
        if col_r.button(f"▶️ Resume from {prior_count:,}", type="primary", use_container_width=True):
    ...
    max_rows = st.number_input("Max leads to score", min_value=10, max_value=n, value=n, step=100)

    if st.button("▶️ Resume Scoring" if has_checkpoint else "🚀 Run Lead Scoring", type="primary", use_container_width=True):
        ...
        start_ts = time.time()
        sc = st.columns(4)
        s_total, s_high, s_warm, s_low = sc[0].empty(), sc[1].empty(), sc[2].empty(), sc[3].empty()

        def update_progress(fraction: float):
            elapsed = time.time() - start_ts
            eta = (elapsed / fraction * (1 - fraction)) if fraction > 0 else 0
            progress_bar.progress(fraction, text=f"Scoring... {fraction*100:.0f}% · ETA {eta:.0f}s")
        ...
        try:
            ...
            add_log(f"Scored {len(scored):,} in {elapsed:.0f}s — High:{high} Warm:{warm} Low:{low}")
            ...
            st.success(f"✅ {len(scored):,} leads in {elapsed:.0f}s — 🔥 {high} · 🟡 {warm} · ❄️ {low}")
...
def page_results_export():
    ...
    filtered = []
    if show_high: filtered.extend(high)
    if show_warm: filtered.extend(warm)
    if show_low:  filtered.extend(low)
    ...
    if permalink_col and permalink_col in df.columns:
        ...
        display_df[permalink_col] = display_df[permalink_col].apply(
            lambda u: f'<a href="{u}" target="_blank">🔗 open</a>' if pd.notna(u) and str(u).startswith("http") else u
        )
    ...
    def to_csv(records):   return pd.DataFrame(records).to_csv(index=False).encode("utf-8")
    def to_jsonl(records): return "\n".join(json.dumps(r, ensure_ascii=False) for r in records).encode("utf-8")

    c1, c2, c3, c4 = st.columns(4)
    c1.download_button("⬇️ All (CSV)",   data=to_csv(scored),  file_name="all_scored.csv",   mime="text/csv",           use_container_width=True)
    c2.download_button("🔥 High (CSV)",  data=to_csv(high),    file_name="high_intent.csv",  mime="text/csv",           use_container_width=True)
    c3.download_button("🟡 Warm (CSV)",  data=to_csv(warm),    file_name="warm_intent.csv",  mime="text/csv",           use_container_width=True)
    c4.download_button("⬇️ All (JSONL)", data=to_jsonl(scored), file_name="all_scored.jsonl", mime="application/jsonl", use_container_width=True)
    ...
    if label_col and label_col in df.columns:
        labels = [f"[{l.get('intent','?').upper()}] {str(l.get(label_col,''))[:60]}" for l in filtered[:200]]
    else:
        labels = [f"[{l.get('intent','?').upper()}] Lead {i}" for i, l in enumerate(filtered[:200])]

    idx  = st.selectbox("Select lead", range(len(labels)), format_func=lambda i: labels[i])
    lead = filtered[idx]

    col1, col2 = st.columns([1, 2])
    with col1:
        intent = lead.get("intent", "?")
        st.metric("Intent", {"high": "🔥 HIGH", "warm": "🟡 WARM", "low": "❄️ LOW"}.get(intent, intent.upper()))
